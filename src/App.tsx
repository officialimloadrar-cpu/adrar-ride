          ...
          {step === 'quote' && (
            <>
              ...
              {type!== 'location'? (
                      ...
                      <>
                        <div className="seats">
                          {([1, 2, 3] as Seats[]).map((s) => (
                            <button key={s} type="button" className={`seat${seats === s? ' active' : ''}`} onClick={() => setSeats(s)}>
                              {s}{s===3?' → 15min':''}
                            </button>
                          ))}
                        </div>
                        {seats === 3 && (
                          <label className="check">
                            <input type="checkbox" checked={fourth} onChange={(e) => setFourth(e.target.checked)} />
                            Add 4th Seat Family +350
                          </label>
                        )}
                      </>
                    ) : (
                ...
                <>
                  <label className="label">Vehicule (avec chauffeur uniquement)</label>
                  ...
                  <p style={{fontSize:'0.75rem', opacity:0.6}}>Location sans chauffeur est desactivee (Pass aussi)</p>
                </>
              )}
          ...
          {step === 'review' && quote && (
            <>
              ...
              <dl className="summary">
                ...
                <div><dt>Seats</dt><dd>{quote.seats}{quote.seats===3?' (15min pour 4e famille)':''}</dd></div>
                <div><dt>Night</dt><dd>{quote.isNight? 'Yes' : 'No'}</dd></div>
                <div><dt>You pay</dt><dd>{quote.finalPrice} DA</dd></div>
                <div><dt>Driver</dt><dd>{quote.driverEarning} DA</dd></div>
                <div><dt>Commission {quote.distance<=10&&quote.type!=='location'?'13%':'15%'}</dt><dd>{quote.commission} DA</dd></div>
              </dl>
              ...
              <div className="search">
                ...
                {quote.seats===3 && (
                  <p style={{marginTop:10, fontSize:'0.9rem'}}>
                    ⏱️ Delai 15min pour ajouter 4e siege famille +350<br/>
...
function PriceBlock({ ride, distance, locked = false }: { ride: { finalPrice: number; driverEarning: number; commission: number; isNight: boolean; type: RideType; seats: number }; distance: number; locked?: boolean }) {
  return (
    <div className="quote">
      ...
      {ride.isNight && ride.type === 'collective' && <span className="badge">Night x1.25 min 250</span>}
      {ride.seats===4 && <span className="badge">4e siege famille = meme personne qui a paye 3 +350</span>}
    </div>
  );
}